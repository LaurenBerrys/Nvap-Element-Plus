 ...
 * @LastEditTime: 2023-11-22 15:44:50
 * @Description: 表单组件
-->
<template>
	<div class="operation">
		<template v-for="(oper, index) in getProps?.operation" :key="index">
			<template v-if="oper.location == 'prefix'">
				<el-button :disabled="oper.disabled" ml-5 v-if="getProps.showSubmitButton" :type="oper.type ? oper.type : 'primary'" @click="oper.method">
	...
	<el-form class="form" style="height: auto" v-bind="getBindValue" :model="formModel" ref="formElRef">
		<el-row>
			<el-col v-show="colShows && index <= colShows" :span="schema.span" v-for="(schema, index) in getSchema" :key="schema.prop" p-1 pb-5>
				<el-form-item v-bind="schema" v-if="!schema.itemShow">
					<!--标签名右侧温馨提示-->
					<template #label v-if="schema.labelMessage">
						{{ schema.label }}
						<el-tooltip :trigger="'hover'" :content="schema.labelMessage" :style="schema.labelMessageStyle">
							<icon-system-uicons:circle-menu />
						</el-tooltip>
					</template>
					<template #label v-else-if="!getBindValue.labelPosition">
						<div w-full flex :class="schema.labelPosition == 'left' ? 'flex-row' : 'flex-row-reverse'">
							{{ schema.label }}
						</div>
					</template>
					<!--判断插槽-->
					...
					<template v-else-if="schema.component === 'ElCheckbox'">
						<el-checkbox-group v-model="formModel[schema.prop]" :disabled="schema.disabled" v-bind="schema.componentProps">
							<el-checkbox v-for="item in schema.componentProps.options" :key="item.value" :value="item.value" :label="item.label" />
						</el-checkbox-group>
					</template>
					<!--单选框Radio-->
					<template v-else-if="schema.component === 'ElRadio'">
						<el-radio-group v-model="formModel[schema.prop]" :disabled="schema.disabled" v-bind="schema.componentProps">
							<el-radio v-for="item in schema.componentProps.options" :key="item.value" :label="item[schema.componentProps.value]">
					...
					<template v-else-if="schema.component === 'ElSelect'">
						<el-select
							...
							:placeholder="
								getProps?.notPlaceholder ? '' : schema.componentProps.placeholder || `请选择${String(schema.label || '').replace(/[:：]/g, '')}`
							"
						...
						>
							<el-option
								...
								:value="schema.componentProps.returnItem ? item : item[schema.componentProps.value] || item.value"
					...
					<CellComponent v-else v-bind="getComponentProps(schema)" :is="schema.component" v-model="formModel[schema.prop]" />
				</el-form-item>
			</el-col>
			<el-col :span="getProps.buttonSpan" flex p-1 pb-5 class="buttonLayout" items-center>
				<el-button ml-5 v-if="getProps.showSubmitButton" :type="getProps.submitButtonType" @click="handleSubmit">
					<TheIcon :icon="getProps.submitIcon" :size="20" style="margin-right: 6px" v-show="getProps.submitIcon" />
					{{ getProps.submitButtonText }}
				</el-button>
				<el-button v-if="getProps.showResetButton" :type="getProps.resetButtonType" @click="resetFields">
					<TheIcon :icon="getProps.resetIcon" :size="20" style="margin-right: 6px" v-show="getProps.resetIcon" />
					{{ getProps.resetButtonText }}
				</el-button>
				<div ml-5>
					<el-text type="primary" style="cursor: pointer" @click="handleSetcolshows" v-if="!coltags">
						{{ coltext }}
...
export default defineComponent({
	...
	setup(props, { emit, attrs }) {
		...
		function getComponentProps(schema) {
			...
			return {
				...
				placeholder: getProps.value?.notPlaceholder
					? ''
					: createPlaceholderMessage(unref(component)) + String(schema.label || '').replace(/[:：]/g, ''),
				...compProps,
			};
		}
		...
		const { handleSubmit, validate, resetFields, getFieldsValue, clearValidate, setFieldsValue, setSchemas, updateProps } = useFormEvents({
			emit,
			getProps,
			formModel,
			getSchema,
