 ...
 * @LastEditTime: 2023-11-24 17:09:52
 * @Description: 表格组件
-->
<template>
      ...
      <template v-for="(item, index) in getBindValues.columns" :key="item">
        ...
        <el-table-column v-else v-bind="item" align="center" show-overflow-tooltip>
          <template #default="scope" v-if="item.ifShow && !item.children">
            <Suspense>
              <slot :name="item.slotName" :row="scope.row">
                <component
                  v-if="item.editRender"
                  :is="item.editRender(scope.row, scope.$index)"
                  :key="index"
                />
          ...
          <template v-if="item.children">
            <el-table-column
              v-for="(ruse, indexs) in item.children"
              :key="indexs"
              v-bind="ruse"
              align="center"
              show-overflow-tooltip
              ...
              <template #default="scope">
                <Suspense>
                  <slot :name="ruse.slotName" :row="scope.row">
                    <component
                      v-if="ruse.editRender"
                      :is="ruse.editRender(scope.row, scope.$index)"
                      :key="index"
                    />
